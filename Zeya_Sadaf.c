#include<stdio.h>
#include<string.h>
#include<stdlib.h>
struct process
{
	int n,i,j,count,t_q,t_slice;
	int a[100],b[100],pr[100],com[100],turn_atime[100],waiting[100],rem_bt[100];
	char c;
}p;
void getdata()
{
P:	printf("Enter the number of processes\n");
	scanf("%d",&p.n);
if(p.n<=0)	
{
	printf("Number of processes entered must be greater than zero\n, Please re-enter\n");
goto P;
}
for(p.i=1;p.i<=p.n;p.i++)
{
Q:	printf("Enter the arrival time for the processes\n");
scanf("%d",a&p.a[p.i]);
if(p.a[p.i]<0)
{
	printf("Arrival Time for the process must not be less than zero\n please enter a valid value\n");
goto Q;	
}
}
for(p.i=1;p.i<=p.n;p.i++)
{
R: printf("Enter the burst time for the processes\n");
scanf("%d",&p.b[p.i]);
if(p.b[p.i]<=0)
{
	printf("The Burst Time entered for the processes should be greater than zero\n");
goto R;
}
}
for(p.i=1;p.i<=p.n;p.i++)
{
	S: printf("Enter the priorities for the processes\n");
	scanf("%d\n",&p.pr[p.i]);
	if(p.pr[p.i]<0)
	{
		printf("Priorities should not be less than zero\n, Please enter a valid choice\n");
goto S;
	}
}
}
void display()
{
	printf("Process number\t\t\tArrival Time\t\t\tBurst Time\t\t\tPriority\n");
	for(p.i=1;p.i<=p.n;p.i++)
	{
	printf("%d\t\t\t   %d\t\t\t   %d\t\t\t  %d\n",p.i,p.b[p.i],p.a[p.i],p.pr[p.i]);
}
}
void priority()
{
	p.count=0;
	A: printf("Enter the Process Id you want to execute:");
	scanf("%d",&p.i);
	if(p.i<=0||p.i>p.n)
	{
		 printf("Invalid input\n");
		 goto A;
		 	}
		 	p.pr[p.i]==0;
		 	if(p.pr[p.i==0])
		 	p.pr[p.i]+=1;
		 	printf("%d is in ready queue\n",p.i);
		 	printf("\nProcess Number\t\t\tBurst Time\t\t\tArrival Time\t\t\tPriority\n");
	printf("    %d\t\t\t\t    %d\t\t\t\t     %d\t\t\t\t    %d\n",p.i,p.b[p.i],p.a[p.i],p.pr[p.i]);
	p.count++;
	fflush(stdin);
	printf("Do you want to execute more process? Y/N : ");
	scanf("%c",&p.c);
	if(p.yes=='Y'||p.yes=='y')
	{
		printf("%d is preempted from the ready queue\n",p.i);
		p.pr[p.i]=2;
		goto A;
	}
	else{
	printf("\n\n\nProcesses in the Waiting queue are \n\n");
	for(p.i=1;p.i<=p.n;p.i++){
	p.count=1;
	if(p.pr[p.i]!=p.count){
	p.pr[p.i]+=2;
	printf("\nProcess Number\t\t\tBurst Time\t\t\tArrival Time\t\t\tPriority\n");
	printf("    %d\t\t\t\t    %d\t\t\t\t     %d\t\t\t\t    %d\n",p.i,p.b[p.i],p.a[p.i],p.pr[p.i]);
				}
			}
		}
	}
}
void timeslice()
{
	p.slice=1;
	int t=0;
	for(p.i=1;p.i<=p.n;p.i++)
	{
		p.rem_bt[p.i]=p.b[p.i];
	}
	while(1)
	{
		bool done=true;
		for(p.i=1;p.i<=p.n;p.i++)
		if(p.rem_bt[p.i]>0){
			done = false;
			if(p.rem_bt[p.i]>p.slice){
				t+=p.slice;
				p.rem_bt[p.i]-=p.slice;
			}
			else{
				t=t+p.rem_bt[p.i];
				p.wat[p.i]=t-p.b[p.i];
				p.rem_bt[p.i]=0;
			}
		}
	if(done==true)
	break;
	}
}
void turnaround()
{
	printf("\n\n\n***************TURNAROUND TIME OF THE PROCESSES***************\n");
	for(p.i=1;p.i<=p.n;p.i++){
	p.tat[p.i]=p.wat[p.i]+p.b[p.i];
	printf("\nProcess Number\t\tBurst Time\t\tArrival Time\t\tPriority\t\tTurnaround Time\n");
	printf("    %d\t\t\t    %d\t\t\t     %d\t\t\t    %d\t\t\t     %d\n",p.i,p.b[p.i],p.a[p.i],p.pr[p.i],p.tat[p.i]);
	}
}
void waiting(){
	printf("\n\n\n****************WAITING TIME OF THE PROCESSES***************\n");
	for(p.i=1;p.i<=p.n;p.i++){
		printf("\nProcess Number\t\tBurst Time\t\tArrival Time\t\tPriority\t\tWaiting Time\n");
	    printf("    %d\t\t\t    %d\t\t\t     %d\t\t\t    %d\t\t\t     %d\n",p.i,p.b[p.i],p.a[p.i],p.pr[p.i],p.wat[p.i]);
	}
}
int main()
{
	float k=0,m=0;
	getdata();
	display();
	priority();
	timeslice();
	turnaround();
	waiting();
	for(p.i=1;p.i<=p.n;p.i++){
	k=k+p.tat[p.i];
	m=m+p.wat[p.i];
	}
	k=k/p.n;
	m=m/p.n;
	printf("Average turnaround time : %f",k);
	printf("\nAverage waiting time : %f",m);
}
